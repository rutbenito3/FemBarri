...
class FemBarriDefSupabaseUser extends BaseAuthUser {
  FemBarriDefSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> femBarriDefSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FemBarriDefSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
